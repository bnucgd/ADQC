...
class ADGate(nn.Module):
    ...
    def __init__(self, name, pos=None, pos_control=None, paras=None, requires_grad=True,
        ...
        self.preprocess()  # 对数据的提前处理
        self.variational = self.requires_grad  # 是否打开自动微分
        ...
        if self.name in ['x', 'y', 'z']:
            ...
            self.paras = paras
            self.tensor = paras  # 直接利用参数定义量子门（参数不可变动）
            self.variational = False  # 不利用 自动微分
# ==================== 以上是非参数门，以下是参数门 ====================
            '''将参数门的参数储存在self.paras中'''
        elif self.name == 'rotate':  # 输入参数门的参数，若无输入，则随机生成
            if paras is None:
            ...
            if paras is None:  # 未输入隐门参数
                if self.pos is None:
                    ndim = 2  # 默认值作用在两个希尔伯特空间的直积空间
                else:
                    ndim = len(self.pos)  # 隐门作用在几个希尔伯特空间上，例：pos = [0, 1, 2], 则作用在三个希尔伯特空间的直积上
                if qudit_dims is None:
                    qudit_dims = [2] * ndim  # 生成算符的shape, 即各个希尔伯特空间维数的列表
                dim_t = math.prod(qudit_dims)  # 根据各个希尔伯特空间的维数生成算符的维数
                if 'initial_way_latent' in self.settings:
                    if self.settings['initial_way_latent'] == 'identity':  # 隐门参数初始化, 默认为单位阵加微扰
                        self.paras = tc.eye(dim_t, dim_t) + 1e-5 * tc.randn((dim_t, dim_t))
                else:
                    self.paras = tc.randn((dim_t, dim_t))  # 未指定隐门参数化方式, 则随机初始化
        ...
        if self.variational:  # 是否打开自动微分
            self.paras = nn.Parameter(self.paras, requires_grad=True)
        self.renew_gate()  # 利用上述（各种if）给出的paras和name定义门

    def preprocess(self):  # 对输入参数的一些处理，将其转换成后续程序要用的类型, 防止类型引起的错误
        if self.name == 'not':
            self.name = 'x'
        if (self.paras is not None) and (type(self.paras) is not tc.Tensor):  # 防止输入的参数不是tensor类型
    ...
    def renew_gate(self):
        '''利用self.paras中参数和self.name中的门的类别创建量子门（形式为tensor）'''
        if self.name == 'phase_shift':
            ...
            self.tensor = self.latent2unitary(self.paras)  # 由隐门生成幺正量子门
...
class ADQC_basic(nn.Module):
    ...
    def act_nth_gate(self, state, n):
        '''受控门作用到tensor上'''
        m_p = len(self.layers[n].pos)  # 作用位的个数
        m_c = len(self.layers[n].pos_control)  # 控制位的个数
        n_qubit = state.ndimension()  # 态的阶数
        shape = state.shape  # 态的shape
        perm = list(range(n_qubit))
        for pp in self.layers[n].pos:
            perm.remove(pp)
        for pp in self.layers[n].pos_control:
            perm.remove(pp)
        perm = self.layers[n].pos + perm + self.layers[n].pos_control  # 作用位放到最前面， 控制位放到最后面

...
class MPS_basic:
    ...
    def center_orthogonalization(self, c, way='svd', dc=-1, normalize=False):  # 中心正交化，c是中心位置
        if c == -1:  # c即中心矩阵要放的位置，-1表明放在最后一位
            c = len(self.tensors) - 1
        if self.center < -0.5:  # 张量不是正交形式
            self.orthogonalize_n1_n2(0, c, way, dc, normalize)
            self.orthogonalize_n1_n2(len(self.tensors)-1, c, way, dc, normalize)
        elif self.center != c:  # 张量是正交形式，但是目标不相同，要改变中心张量的位置
            self.orthogonalize_n1_n2(self.center, c, way, dc, normalize)
        self.center = c
        if normalize:
            self.normalize_central_tensor()  # 归一化中心张量
    ...
    def check_center_orthogonality(self, prt=True):
        if self.center < -0.5:
            ...
            err = check_center_orthogonality(self.tensors, self.center, prt=prt)  # 使用另外定义的函数，名字同该函数名字相同
            return err  # 返回一个列表

    def full_tensor(self):  # 得到全局张量
        return full_tensor(self.tensors)

    def inner(self, tensors, form='log'):  # 计算张量内积
    ...
    def move_center_one_step(self, direction, decomp_way, dc, normalize):  # 移动中心位置
        if direction.lower() in ['right', 'r']:
            if -0.5 < self.center < self.length-1:  # 中心正交形式且中心不在最右边
            ...
            if self.center > 0:  # 中心正交形式且中心不在最左边
    ...
    def normalize_central_tensor(self, normalize=True):  # 归一化中心张量
        norm = self.tensors[self.center].norm()
        if normalize:
            self.tensors[self.center] = self.tensors[self.center] / norm
        return norm  # 返回中心张量的模

    def norm_square(self, normalize=False, form='inner'):  # 求取张量模方
        if self.center > -0.5:  # 中心正交形式
            norm = self.normalize_central_tensor(normalize=normalize)
            if form == 'inner':
                norm = norm ** 2  # 全局张量的模方等于中心张量的模方
            else:  # 返回模方的log
                norm = 2 * tc.log(norm)
        else:  # 非中心正交形式
            norm, self.tensors = norm_square(self.tensors, normalize=normalize, form=form)  # 采用的其他定义的函数，与该函数同名
        return norm

    def normalize(self):  # 对MPS归一化，并返回MPS归一化前的模
        norm, self.tensors = norm_square(self.tensors, normalize=True, form='list')  # 采用类外的函数对MPS归一化，并返回模
        return norm
    ...
    def orthogonalize_left2right(self, nt, way, dc=-1,
                                 normalize=False):  # 将中心张量的位置向右移动一位
        # dc=-1意味着不进行裁剪
        assert nt < len(self.tensors)-1  # 判断是否可以从左到右正交化
        s = self.tensors[nt].shape  # 中心张量的形状
        if 0 < dc < s[-1]:
        ...
        tensor = self.tensors[nt].reshape(-1, s[-1]).to('cpu')  # 将中心张量转移到cpu，便于后续svd
        if way.lower() == 'svd':
            u, lm, v = tc.linalg.svd(tensor,
                                     full_matrices=False)  # 分解中心张量
            lm = lm.to(dtype=u.dtype)
            if if_trun:  # 需要裁剪
                u = u[:, :dc].to(self.device)  # 只取前几个左奇异向量
                r = tc.diag(lm[:dc]).to(self.device).mm(v[:dc, :].to(self.device))  # 去除左变换矩阵（裁剪后）
            else:
                r = tc.diag(lm).to(self.device).mm(v.to(self.device))
        else:
            u, r = tc.linalg.qr(tensor)  # 使用QR分解(效率更高，但是无法进行裁剪)
            lm = None
            u, r = u.to(self.device), r.to(self.device)
        self.tensors[nt] = u.reshape(s[0], s[1], -1)  # 左虚拟指标和物理指标不能变，自动计算右虚拟指标
        if normalize:
            r /= tc.norm(r)
        self.tensors[nt+1] = tc.tensordot(
            r, self.tensors[nt+1], [[1], [0]])  # 得到新的tensor[nt+1]
        return lm  # 返回中心向量的奇异值谱（二分纠缠谱）
    ...
    def orthogonalize_n1_n2(self, n1, n2, way, dc, normalize):  # 将中心张量位置从n1移动到n2
        if n1 < n2:
...
def norm_square(tensors, normalize=False, form='log'):
    # norm = <psi|psi>
    assert tensors[0].shape[0] == tensors[-1].shape[-1]  # 保证虚拟指标可以被缩并掉
    v = tc.eye(tensors[0].shape[0]**2, dtype=tensors[0].dtype, device=tensors[0].device
               ).reshape([tensors[0].shape[0]] * 4)   # 创建一个四阶的单位张量
    ...
    for n in range(len(tensors)):
        ...
        v = v / norm_list[-1]
        if normalize:
            tensors[n] = tensors[n] / tc.sqrt(norm_list[-1])  # 对局域张量进行归一化
    if v.numel() > 1:  # 说明两端的虚拟指标的维数不是一，即为周期MPS
        norm1 = tc.einsum('acac->', v)
        norm_list.append(norm1)
        if normalize:

...
def act_on_state(state, gate, left_to_right=True, vd = None):
    ...
    if left_to_right:
        for i in range(len(state) - 1):
            ...
            lm = tc.tensor([i for i in lm if i >= 1e-10])
...
for i in range(iter_times):
    '''建立初态MPS'''
    original_state = copy.deepcopy(phi)
    '''生成量子门'''
    ...
    for j in range(layers):
        gate_ = gates[j * (nq - 1): (j + 1) * (nq - 1)]
        act_on_state(state=original_state, gate=gate_, left_to_right=((j % 2) == 0), vd=48)
    ...
    '''打印loss'''
    if (i+1) % 20 == 0:
        print('第%d次迭代, loss=' % (i + 1), loss.item(), '保真度：', math.exp(-loss * nq))
